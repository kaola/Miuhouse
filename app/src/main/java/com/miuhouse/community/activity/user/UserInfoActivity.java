...
import com.miuhouse.community.bean.HeadUrl;
import com.miuhouse.community.bean.StatusBean;
...
import com.miuhouse.community.utils.MyAsyn;
import com.miuhouse.community.utils.MyUtils;
import com.miuhouse.community.utils.StringUtils;
import com.miuhouse.community.utils.UpdatePhotoUtils;
...
//                isBitmap = true;
    ...
    private void initFill() {
        ...
        if (mUserBean.getHeadUrl() != null) {
            ...
            Glide.with(this).load(strHeadUrl).placeholder(R.mipmap.ic_launcher).override(MyUtils.dip2px(this, 50), MyUtils.dip2px(this, 50)).into(imgAvatar);
        }
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            case R.id.relative_nicename:
                startActivityForResult(new Intent(this, UpdateNiceNameActivity.class).putExtra("niceName", tvNicename.getText().toString()), NICE_NAME);
                break;
            case R.id.relative_name:
                startActivityForResult(new Intent(this, UpdateNameActivity.class).putExtra("name", tvName.getText().toString()), NAME);
                break;
            case R.id.relative_build:
                startActivityForResult(new Intent(this, UpdateBuildActivity.class).putExtra("Unit", intUnit).putExtra("Build", intBuild), BUILD);
                break;
            case R.id.relative_phone:
                startActivityForResult(new Intent(this, UpdatePhoneActivity.class).putExtra("phone", tvPhone.getText().toString()), PHONE);
                break;
                ...
                new Thread() {
                    ...
                    public void run() {
                        ...
                        if (RESULT_LOAD_IMAGE == requestCode) {
                            ...
                            UpdatePhotoUtils.startPhotoZoomOne(selectedImageUri, UserInfoActivity.this, imageAddress);
                            ...
                            if (requestCode == TAKE_PICTURE) {
                                ...
                                // 拍摄图片
                                imagePath = UpdatePhotoUtils.getImagePath(imageAddress);
                                UpdatePhotoUtils.startPhotoZoomOne(photoUri, UserInfoActivity.this, imageAddress);
...
//                                    }
    ...
    private void sendRequest() {
        ...
        GsonRequest<StatusBean> custom = new GsonRequest<>(Request.Method.POST, urlPath, StatusBean.class, map, getVerifyListener(), new ErrorCallback());
    ...
    private Response.Listener<StatusBean> getVerifyListener() {
        return new Response.Listener<StatusBean>() {
            ...
            public void onResponse(StatusBean baseBean) {
                ...
                UserInfoActivity.this.sendBroadcast(new Intent(LoginActivity.INTENT_ACTION_USER_CHANGE));
                finish();
            }
        };
    }
